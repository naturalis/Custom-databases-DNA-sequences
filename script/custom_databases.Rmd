...
Computing and drawing a collapsible tree, and its datatable.
...
server <- function(input, output, session) {
  ...
  observeEvent(input$Hierarchy,{
    output$d3 <- renderD3tree({
     ...
     d3tree(data = list(root = df2tree(struct = p,rootname = 'taxa_unique'), layout = 'collapse'),activeReturn = c("phylum", "class", "order", "family", "genus", "species","bold_records"),height = 18)
    })
})
