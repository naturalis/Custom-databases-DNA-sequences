...
will be extracted from a database of choice.
...
for(x in 1:length(treeOut)){
  tryCatch({
    ...
    row <- data.frame(cbind(phylum=c.phylum,class=c.class,order=c.order,
                            family=c.family,genus=c.genus,species=c.species,taxID=c.taxID))
...
taxdata_full <- left_join(taxidOut, taxdata, by=c("id" = "taxID")) %>%
  left_join(., species, by=c("name" = "species_name")) %>%
  tibble::rowid_to_column(., "taxdata_id") %>%
  .[, c(1,10,9,4,5,6,7,8,2,3)] # Reaarange columns

# Adjust Header
names(taxdata_full) <- c("taxdata_id", "node_id", "node_name", "phylum", "class", "order",
                         "family", "genus", "species", "taxonomy_id")
...
vis.tree <- inner_join(species, taxdata_full[c(2,4:8)], by=c("species_id" = "node_id")) %>%
