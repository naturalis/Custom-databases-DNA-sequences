...
## Remove NAs, sum unique species record counts, add DB/marker identifier
...
natSpecies <- natSpecies[,c(5,2,3,4)]
colnames(natSpecies)[1] <- "species"

## Match species against the NSR
natCoverage <- left_join(nsrSpecies[c(2:3)], natSpecies, by=c("species_name"="species"))
...
ncbi_tree <- left_join(taxdata, taxidOut, by=c("tax_id"="id"),
                       suffix=c("","_nsr")) %>%
  left_join(., nsrSpecies[1:2], by=c("name_nsr" = "species_name")) %>%
  .[, c(1,6,2,3,4)] # Reaarange columns, drop 'name_nsr' column
ncbi_tree <- ncbi_tree[complete.cases(ncbi_tree$species_id), ]

## Write NCBI data to file
write.csv(ncbi_tree, file=sprintf('%s/results/tree_ncbi.csv', REPO_HOME),
          row.names=FALSE)
...
## Add a unique identifier for each record, along with BOLD's database ID
species_markers <- within(species_markers, {
  sm_id <- seq.int(nrow(species_markers))
  database_id <- 1
}) %>%
  .[, c(8,1,7,6,5)] %>% # Remove obsolete and rearrange columns
...
```{r visTree, include=FALSE}
## Get classification for each species name
## Subsequently, add coverage of BOLD records
...
boldTree <- boldTree[,c(4,5,6,7,8,9,2,12,13)]
colnames(boldTree)[7] <- "species"

## Add count for unique BOLD records
...
## Create similar tree for Naturalis records
natTree <- left_join(nsrBackbone[c(1:7)],
                       natCoverage[c('species_name','counts','database_name','marker_name')],
                       by=c("species"="species_name"))

## Merge BOLD & Naturalis trees
visTree <- unique(merge(boldTree, natTree,all=TRUE)) %>% drop_na(counts)

## Add NSR's unmatched species to the combined tree
visTree <- right_join(visTree, nsrBackbone[c(1:7)],
...
## Define tree data ----
m=visTree%>%data.frame%>%mutate(NEWCOL=NA)%>%distinct
