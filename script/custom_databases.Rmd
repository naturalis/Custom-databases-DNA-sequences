...
## Cross-check and isolate matching species for specimen records
nsrTotal <- full_join(nsrSpecies[c(2:3)], nsrSynonyms[c(2:3)],
                    by=c("species_name"="synonym_name",
                         "identification_reference"))
natCoverage <- left_join(nsrTotal, natSpecies, by=c("species_name"="species"))
...
server <- function(input, output, session) {
  ...
  output$billboarder <- renderBillboarder(expr = {
    ...
    billboarder() %>%
      bb_barchart(
        data = vis.cov[, c("taxa", "bold_total", "nat_total")]
      ) %>%
      bb_data(
        names = list(bold_total = "bold", nat_total = "naturalis")
      ) %>%
      bb_y_grid(show = TRUE) %>%
